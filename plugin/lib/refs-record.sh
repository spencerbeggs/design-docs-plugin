...
#
# Run from the repository root (paths in refs.json are repo-root-relative).
#
# Usage:  refs-record.sh <claude-md> [refs.json]
#         (refs.json defaults to .claude/design/refs.json)
...
source_file="$1"
refs="${2:-.claude/design/refs.json}"
...
if [[ ${#rel_targets[@]} -gt 0 ]]; then
	for rel in "${rel_targets[@]}"; do
		abs="$base_dir/$rel"
		d="$(cd "$(dirname "$abs")" 2>/dev/null && pwd)" || {
