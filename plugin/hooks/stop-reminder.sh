...
# Output soft nudge as JSON. Stop hooks do not support hookSpecificOutput.additionalContext;
# use top-level systemMessage instead (per the Stop hook output schema).
...
jq -n --arg msg "$NUDGE_MSG" '{ systemMessage: $msg }'
